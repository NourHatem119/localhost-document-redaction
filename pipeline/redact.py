...
def apply_redactions(
    ...
    char_bboxes : dict, optional
        ...
        If provided, text offsets are mapped directly to page bboxes.
        If None, falls back to `search_for()` on each page.
    ...
    for span in job.spans:
        if char_bboxes:
            # Find all page bboxes for this span from the lookup table.
            bboxes = []
    ...
    # Group image regions by page.
    regions_by_page: dict[int, List[ImageRegion]] = {}
    for region in job.regions:
        regions_by_page.setdefault(region.page_no, []).append(region)
    ...
    for page_no in range(len(doc)):
        ...
        # Text redactions.
        for span in spans_by_page.get(page_no, []):
            if char_bboxes:
                rects = []
                for offset in range(span.char_start, span.char_end):
                    key = (span.doc_id, offset)
                    if key in char_bboxes:
                        for _, bbox in char_bboxes[key]:
                            rects.append(_to_fitz_rect(bbox))
                # Merge overlapping rects for the same span.
                if rects:
                    union = rects[0]
                    for r in rects[1:]:
                        union |= r
